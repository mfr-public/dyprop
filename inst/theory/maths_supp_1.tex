...
Standard compositional methods (e.g., \texttt{propr}) assume static relationships. To model dynamics, we introduce a continuous pseudotime coordinate $t \in [0,1]$ and derive the \textbf{Instantaneous Weighted Covariance}.
...
We define $\Phi(t)$ as the magnitude of proportionality deviation relative to the variance of the reference feature.
\begin{equation}
\Phi_{jk}(t) = \frac{\text{var}_w(\log(x_j/x_k))_t}{\text{var}_w(\text{clr}(x_j))_t}
\end{equation}
High $\Phi$ indicates a decoupling event.
...
\subsection{The Boundary Function Dictionary}
...
\section{Dynamical Systems Interpretation: Landscape Stiffness}

\subsection{The Potential Landscape}
We interpret the Gene Regulatory Network (GRN) as a dynamical system moving on a quasi-potential landscape $U(\mathbf{y})$, where $\mathbf{y}$ is the vector of log-ratios. In a stable homeostatic state, the system resides in a deep basin of attraction. We approximate the local potential as a harmonic oscillator around the equilibrium $\theta$:
\begin{equation}
    U(y) \approx \frac{1}{2} k (y - \theta)^2
\end{equation}
where $k$ is the \textbf{Regulatory Stiffness} (curvature) of the landscape.

\subsection{Variance as Inverse Stiffness}
Assuming the system is subject to stochastic noise (transcriptional bursting) of magnitude $D$, the variance of the log-ratio is derived as:
\begin{equation}
    \text{Var}(y) = \frac{D}{k}
\end{equation}

\subsection{Dynamic Instability as a Critical Signal}
Consequently, our metric $\Phi(t)$ serves as a proxy for the \textbf{Inverse Stiffness} ($k^{-1}$).
\begin{itemize}
    \item \textbf{Homeostasis ($\Phi \approx 0$):} High stiffness ($k \gg D$). Strong repression minimizes noise.
    \item \textbf{Transition ($\Phi \gg 0$):} Loss of stiffness ($k \to 0$). The system traverses a ``saddle point'' (flattened landscape) to facilitate rewiring \citep{scheffer2009}.
\end{itemize}
Thus, DyProp identifies network events not by expression level, but by the collapse of the regulatory constraints that define the cell state.
...
\bibitem{lovell2015}
Lovell, D., et al. (2015).
Proportionality: A valid alternative to correlation for relative data. \textit{PLoS Comp Bio}.

\bibitem{erb2016}
Erb, I., \& Quinn, T.P. (2016).
Differential Proportionality. \textit{Analysis of Large and Complex Data}.
